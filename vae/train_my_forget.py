...
import pickle
from model import OneHotCVAE, loss_function, Classifier
...
def parse_args_and_ckpt():
    ...
    parser.add_argument(
        ...
        help="Gamma hyperparameter for contrastive term in loss (left at 1 in main paper)"
    )

    parser.add_argument(
        "--n_iters", type=int, default=100000, help="Number of iterations"
    )
...
def train():
    ...
    for step in range(0, args.n_iters):
        ...
        selected = torch.from_numpy(np.random.choice(label_choices, size=args.batch_size)).to(device)
        c_remember = F.one_hot(selected, 10)
        ...
        with torch.no_grad():
            ...
            # out_forget = vae.decoder(z_forget, c_forget).view(-1, 1, 28, 28)
        ...
        out_forget = vae.decoder(z_forget, c_forget).view(-1, 1, 28, 28)
        # loss = loss_function(recon_batch, torch.rand((args.batch_size, 1, 28, 28)).to(device), mu, log_var)
        log_probs = classifier(out_forget.view(args.batch_size, 1, 28, 28)) # model outputs log_softmax
        probs = log_probs.exp()
        negative_loss = probs[:, args.label_to_drop].sum()
        # negative_loss = loss

        # contrastive loss
        recon_batch, mu, log_var = vae(out_remember, c_remember)
        log_probs = classifier(out_remember) # model outputs log_softmax
        probs = log_probs.exp()
        positive_loss = -probs[torch.arange(0, probs.size(0), device=probs.device), selected].sum()
        # positive_loss = 0
        recon_loss = args.gamma * loss_function(recon_batch, out_remember, mu, log_var)
        loss = negative_loss + positive_loss + recon_loss
        ...
        if (step + 1) % args.log_freq == 0:
            ...
            logging.info('Negative Loss: {:.6f}\t Positive Loss: {:.6f}\t Avg Forgetting Loss: {:.6f}\t Avg EWC Loss\n: {:.6f}'.format(
                negative_loss, positive_loss, forgetting_loss, ewc_loss
            ))
            sample(step)
            train_loss = 0
            forgetting_loss = 0
